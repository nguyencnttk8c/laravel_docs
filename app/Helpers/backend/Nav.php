...
class Nav{
    public static function multiLevel($value,$menuData){
     	$html = '';
    	...
    	$html.= "<li class=\"".((in_array($currentRoute,$routes))?'open':NULL)."\">
            ...
            foreach($menuData['parent'][$value] as $child){
                if(isset($menuData['parent'][$child])){    
                	$html .= self::multiLevel($child,$menuData);
                }else{
    ...
    public static function results(){
        ...
        return $html = '<ul class="nav nav-list">'
            .self::rederHtml(0,$menuData).
        '</ul>';
    }
}
?>